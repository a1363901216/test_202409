...
import get_ori_data, get_signal
from trader_time.data_feed import MyDataFeed
...
if __name__ == '__main__':
    data_feed = MyDataFeed(isTest=True, reload_from_clickhouse=False)

    stock_all, shangzheng = get_ori_data.load(isTest=False, reload_from_clickhouse=True)
    buy_signal = get_signal.buy_signal(code_2_detail=stock_all)
    ...
    i = 0
    trade_date = stock_all['trade_date'].unique()
    buy_date_to_group = {trade_date: group for trade_date, group in buy_data.groupby('trade_date')}
    for trade_date_one in trade_date:

        i = i + 1
        if i % 100 == 0:
            print("buy_data.groupby ", trade_date_one)

        my_orders_dict = {}
        if trade_date_one in buy_date_to_group:
            group = buy_date_to_group[trade_date_one]
            my_orders_dict = {row.ts_code: row for row in group.itertuples()}

        my_broker.next(my_orders_dict=my_orders_dict, trade_date=trade_date_one)
        # else:
        #     my_broker.next(dict, {}, trade_date)
    my_broker.checkout()
    print("my_broker.money ", my_broker.money)
    print("my_broker.money cost", time.time() - now)

    my_broker.plot_money(shangzheng)
